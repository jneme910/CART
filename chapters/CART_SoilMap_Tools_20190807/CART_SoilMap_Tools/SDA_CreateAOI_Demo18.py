...
## ===================================================================================
def CreateAOILayer(inputAOI, bDissolve, bClean, aoiShp):
    # Create new featureclass to use in assembling AOI query for Soil Data Access
    ...
    try:
        ...
        if bDissolve:
            # User wants to dissolve to single part polygons
            PrintMsg(" \nDissolving unneccessary polygon boundaries for the AOI...", 0)
            arcpy.Dissolve_management(simpleShp, aoiShp, "", "", "SINGLE_PART") 
            ...
            if ("LAND_UNIT_TRACT_NUMBER" in fldNames and "LAND_UNIT_LAND_UNIT_NUMBER" in fldNames):
                ...
                #PrintMsg(" \nUsing landunit polygon attributes to build AOI for Web Soil Survey", 0)
                arcpy.Dissolve_management(simpleShp, aoiShp, "landunit", "", "MULTI_PART")
                blandunit = True
                ...
                #PrintMsg(" \nUsing CLU layer to build AOI for Soil Data Access query", 0)
                arcpy.AddField_management(simpleShp, "landunit", "TEXT", "", "", 20)
                curFields = ["landunit", "FARM_NUMBE", "TRACT_NUMB", "CLU_NUMBER"]

                with arcpy.da.UpdateCursor(simpleShp, curFields) as cur:
                    for rec in cur:
                        # create stacked label for tract and field
                        #landunit = "F" + str(rec[1]) + "\nT" + str(rec[2]) + "\nN" + str(rec[3])
                        landunit = "T" + str(rec[2]) + " Fld" + str(rec[3])
                        rec[0] = landunit
                        cur.updateRow(rec)
                ...
                #PrintMsg(" \nUsing CLU layer to build AOI for Soil Data Access query", 0)
                arcpy.AddField_management(simpleShp, "landunit", "TEXT", "", "", 20)
                curFields = ["landunit", "FARM_NUMBER", "TRACT_NUMBER", "CLU_NUMBER"]
                ...
                curFields = ["landunit", "FARMNBR", "TRACTNBR", "CLUNBR"]
...
def FormSDA_Queries(aoiDiss):
    ...
    # bPropertyQueries 
    # bDiagnosticQuery
    # SSURGO: sdvattribute.tiebreakdomainname(40), mdstatdomdet.domainname, mdstatdomdet.choicesequence
    # SDM: MetadataDomainMaster.DomainID, MetadataDomainMaster.DomainName, MetadataDomainDetail.DomainID,
    # MetadataDomainDetail.ChoiceSequence, MetadataDomainDetail.ChoiceName
    try:
...
-- #LandunitRatingsDetailed1 table columns: landunit, attributename, rating_class, rating_value, rating_key, rating_pct, rating_acres, landunit_acres
...
-- #LandunitRatingsDetailed2 table columns: landunit, attributename, rating_class, rating_value, rating_key, rating_pct, rating_acres, landunit_acres, rolling_pct, rolling_acres 
...
-- #LandunitRatingsCART table columns: id, landunit, attributename, rating_class, rating_value, rating_key, rolling_pct, rolling_acres, landunit_acres
...
-- #LandunitRatingsCART2 table columns: id, landunit, attributename, rating_class, rating_value, rating_key, rolling_pct, rolling_acres, landunit_acres
...
-- I would like to incorporate the same information (and perhaps more) for soil properties.
-- The table can incorporate everything thing that sdvattribute contains.
...
-- Update this list 1ist!!! #M4 columns: aoiid, landunit, mukey, mapunit_acres, cokey, compname, comppct_r, majcompflag
INSERT INTO #M4
...
"""
        ...
        if bCartQueries or bInterpQueries:
            ...
            PrintMsg(" \n\tAdding Soil Property Queries", 0)
            sPropertyQuery = """
-- ************************************************************************************************
...
comppct_r, majcompflag, mu_pct_sum, major_mu_pct_sum, drainagecl, (1.0 * comppct_r / mu_pct_sum) AS adj_comp_pct 
FROM #M4 
...
-- SELECT landunit, aoiid, mapunit_acres , mukey, cokey, CAST ((1.0 * comppct_r / mu_pct_sum) AS DEC(5, 2)) AS adj_comp_pct
SELECT landunit, aoiid, mapunit_acres , mukey, cokey, (1.0 * comppct_r / mu_pct_sum) AS adj_comp_pct
INTO #muacpf
FROM #acpf AS acpf2
...
-- NEED TO CONFIRM NEGATIVE ESP_L VALUES WITH JASON
INSERT INTO #agg1
SELECT DISTINCT 
MA44.aoiid ,
MA44.landunit, 
...
ec_r,
ec_h,
-- CAST (CAST ((100 * (-0.0126 + 0.01475 * sar_l))/(1 + (-0.0126 + 0.01475 * sar_l)) AS FLOAT)  AS DEC(10, 2)) AS esp_l,
-- CAST ((100 * (-0.0126 + 0.01475 * sar_l))/(1 +(-0.0126+0.01475*sar_l)) AS DEC(10, 2)) AS esp_l,
...
tcl, major_mu_pct_sum, mu_pct_sum, (1.0 * comppct_r / mu_pct_sum) AS adj_comp_pct, CASE WHEN hzdepb_r IS NULL THEN 0
...
--
INSERT INTO #o1
SELECT DISTINCT og.aoiid, og.landunit, landunit_acres, mukey, mapunit_acres, cokey, cname, copct, majcompflag, mu_pct_sum, CAST ((1.0 * copct / mu_pct_sum) AS DEC(10, 2)) AS adj_comp_pct, 
taxgrtgroup,
taxsubgrp,
hydricrating 
FROM #AoiAcres
...
-- Hydric Interp Ratings begins here

-- Populate #Hydric_A with component-level hydric percent ratings
...
-- End Jasons new hydric interpretation queries here
...
"""
        ...
        if bInterpQueries or bCartQueries :
            ...
            for attributeName in attributeList:
                time.sleep(1)
                PrintMsg(" \n\tAdding query for '" + attributeName + "'", 0)
...
TRUNCATE TABLE #M6
INSERT INTO #M6
SELECT aoiid, landunit, mukey, mapunit_acres, cokey, compname, comppct_r, rating_class, mu_pct_sum, (1.0 * comppct_r / mu_pct_sum) AS adj_comp_pct
FROM #M5
;
...
-- Detailed Landunit Ratings2 table columns: landunit, attributename, rating_class, rating_key, rating_value, rating_pct, rating_acres, landunit_acres, rolling_pct, rolling_acres 
...
"""
        ...
        if bDiagnosticQuery:
            ...
            sQuery += """
-- Farm Class LUD
-- LandunitFarmLandClass - detailed acres for each rating class by landunit
...
-- ***
--  SOC LUD
-- SELECT DISTINCT  landunit, landunit_acres, 'Soil Organic Carbon Stock' AS rating_name,
...
-- Farm Class LUD
-- LandunitRatingsFarmClass
-- In the soils database mapunit table, the 'Not rated' were originally NULLs.
...
-- Hydric LUD  ***
--
-- LandUnitRatingsHydric
--  SDP - Turn this one off for now, but add some of it to the LandunitRatingCART2 table
-- SELECT DISTINCT #Hydric3.landunit, 'Hydric Soils' as rating_name,
-- CASE WHEN rv_acres IS NULL THEN 2 
-- WHEN rv_acres  > 0 THEN 1
-- WHEN rv_acres = 0 THEN 0
-- END AS rating_value_hydric,
...
-- Hydric LUD
-- Returns low, rv and high ratings as acres and percent
-- SELECT DISTINCT landunit, attributename,
-- ROUND (SUM (low_acres) OVER(PARTITION BY aoiid), 2) AS aoiid_low_acres, 
-- ROUND (SUM (rv_acres) OVER(PARTITION BY aoiid), 2) AS aoiid_rv_acres, 
-- ROUND (SUM (high_acres) OVER(PARTITION BY aoiid), 2) AS aoiid_high_acres,
-- ROUND((ROUND (SUM (low_acres) OVER(PARTITION BY aoiid), 2) / aoi_acres) * 100.0, 2) AS aoiid_low_pct, 
...
-- SDV attribute table needed to create soil interp map layers 
-- 
...
-- Begin saving the final CART soil interpretation ratings for each landunit
-- Note. The LandunitRatingsCART table will have all data, but the record for the overall landunit rating will have an id = 1.
...
-- Ponding or Flooding
-- Water Table
-- SELECT landunit, attributename As rating_name, rating_value AS rating_value 
-- FROM #LandunitRatingsCART2
-- ORDER BY landunit, attributename
--;
 
-- For ArcMap LandUnit Rating table, also return rating_class.
-- Problem. Some rating_class values (standard interps) are stored in #RatingDomain and
-- some are assigned in the query and written to #LandunitRatingsCART2.
--
SELECT landunit, LRC.attributename AS rating_name, LRC.rating_value, rating_class
FROM #LandunitRatingsCART2 LRC
ORDER BY landunit, LRC.attributename
-- LEFT OUTER JOIN #RatingDomain RD ON LRC.attributename = RD.attributename AND LRC.rating_value = RD.rating_value
-- ORDER BY landunit, LRC.attributename
;
...
def RunSDA_Queries(theURL, sQuery, gdb):
    ...
    try:
        ...
        if not "Table" in data:
            raise MyError, "No data returned for this request"

        else:
            bSpatial = False  # flag to indicate spatial data was returned
            
            keyList = sorted(data.keys())
            # PrintMsg(" \nJSON data contains these items: " + ", ".join(keyList), 1)
            ...
            for key in keyList:
                ...
                if "wktgeom" in columnNames:
                    ...
                    if newTableName == "xxxSoilMap_by_Landunit":
                        ...
                        soilsLayer = arcpy.mapping.Layer(tmpLayerFile)
                        soilsLayer.name = newTableName
                        soilsLayer.visible = False
                        arcpy.mapping.AddLayer(df, soilsLayer, "TOP")
                        PrintMsg(" \n\tRunSDA_Queries:  Adding new featurelayer to ArcMap: " + newTableName, 0)

                else:
                    # no geometry present, create standalone table
                    #
                    if "id" in columnNames and "landunit" in columnNames and "rating_key" in columnNames and "rolling_acres" in columnNames:
                    ...
                    elif len(columnNames) == 4 and "landunit" in columnNames and "rating_name" in columnNames and "rating_value" in columnNames and "rating_value" in columnNames:
                        newTableName = "LandunitRatingsCART"
                        ...
                        newTableName = "MapunitComponents"
                    ...
                    newTable = os.path.join(gdb, newTableName)
                    #PrintMsg(" \nCreating new table (" + newTableName + ") from object: " + key, 0)
                ...
                if "wktgeom" in columnNames:
                    ...
                    if newTableName != "SoilMap_by_Landunit":
                        ...
                        soilsLayer.name = newTableName
                        arcpy.mapping.AddLayer(df, soilsLayer)
                    ...
                    viewTbl = arcpy.mapping.TableView(newTable)
                    arcpy.mapping.AddTableView(df, viewTbl)
...
def CreateGroupLayer(grpLayerName):
    try:
        ...
        grpLayer.description = "Group layer containing individual soil maps " 
...
def CreateInterpMaps(gdb, soilsLayer, tableList):
    ...
    try:
        ...
        if "MapunitInterps_DCD" in tableList and "SDV_Attribute" in tableList and "SoilMap_by_Landunit" in tableList:
            ...
            if len(attsList) > 0:
                PrintMsg(" \nCreating soil interpretation maps...", 0)
            
            for sdvAtt in attsList:
                #sdvAtt = sdvAtt
                outputValues = dOutputValues[sdvAtt]
                #PrintMsg(" \nCreating a View for " + sdvAtt + " in MapunitInterps_DCD", 1)
                # datasetName, dataSource, name, isBroken, definitionQuery


                # Try creating the soil map layers without creating these table views. Trying to speed it up.
                interpTbl = arcpy.mapping.TableView(saTable)
                interpTbl.name = "View_" + sdvAtt.replace(" ", "_")
                interpTbl.definitionQuery = "attributename = '" + sdvAtt + "'"
                #arcpy.mapping.AddTableView(df, interpTbl)
                #defQuery = "attributename = '" + sdvAtt + "'"
                ...
                outputLayerFile = os.path.join(env.scratchFolder, "SDV_" + sdvAtt.replace(" ", "_") + ".lyr")
                ...
                # soilsLayer is a mapping Layer object
                # interpTbl is the table view containing ratings for the current interp
                # outputLayerName is the name of the soils interp map is to be created (sttring
                # outputLayerFile is the full path and filename for the .lyr file to be created from the outputLayer
                # outputValues is a list of rating values for this interp
                # parameter string - runtime settings from dSDV to be used for metadata
                # creditsString - runtime settings from dSDV to be used for metadata
                ...
                # The CreateMapLayer function has been altered so that it now just adds the new sdvAtt column to the soils featureclass and populates it
                #
                #PrintMsg(" \ndLayerDefinition: " + str(dLayerDefinition), 1)
                
                #bMap = CreateMapLayer(soilsFC, interpTbl, outputLayerName, outputLayerFile, outputValues, parameterString, creditsString, dLayerDefinition, False)
                bMap = CreateMapLayer(soilsFC, interpTbl, outputLayerName, outputLayerFile, outputValues, parameterString, creditsString, dLayerDefinition, False)
                del interpTbl
                ...
                arcpy.MakeFeatureLayer_management(soilsFC, outputLayerName)

                sdvLyr = "SDV_PolygonUnique.lyr"
                sdvLyrFile = os.path.join(os.path.dirname(sys.argv[0]), sdvLyr)
                symbologyLayer = arcpy.mapping.Layer(sdvLyrFile)  # layer with unique values symbology

                arcpy.SaveToLayerFile_management(outputLayerName, outputLayerFile, "RELATIVE", "10.3")
                time.sleep(1)
                arcpy.Delete_management(outputLayerName, "FeatureLayer")

                interpLayer = arcpy.mapping.Layer(outputLayerFile)  # Basic soils layer
                
                arcpy.mapping.UpdateLayer(df, interpLayer, symbologyLayer, True)
                
                interpLayer.symbology.valueField = newFieldName.encode('ascii')
                interpLayer.symbology.addAllValues()

                #PrintMsg(" \nInterp dLayerDefinition: " + str(dLayerDefinition), 1)
                ...
                arcpy.mapping.AddLayerToGroup(df, grpLayer, interpLayer, "BOTTOM")  # add soil map layer to group layer
                #arcpy.mapping.AddLayer(df, interpLayer, "TOP")


            interpLayer.visible = True
            #arcpy.RefreshTOC()
...
## ===================================================================================
def AddFirstSoilMap(outputFC, newLayerFile, labelField):
    ...
    try:
        ...
        valList = GetUniqueValues(outputFC, labelField)
        newLayer = arcpy.mapping.Layer(newLayerFile)  #fix this
        newLayer.name = newLayerName # from global variable
        
        #arcpy.mapping.AddLayerToGroup(df, grpLayer, newLayer, "TOP")
        ...
        mapAngle = RotateMap(mxd, df)

        # Update soilmap layer symbology using JSON dictionary
        # Having problems.... Do I need to AddLayer??
        installInfo = arcpy.GetInstallInfo()
        version = str(installInfo["Version"])
        #bLabeled = AddLabels(newLayer, labelField, True, 12)
        
        if version[0:4].startswith("10."):
            if newLayer.supports("SYMBOLOGY"):
                #PrintMsg(" \n\tUpdating soil mapunit unique values symbology using JSON string", 1)
                #time.sleep(5)

                # Update layer symbology using JSON
                #dLayerDefinition = SimpleFeaturesJSON()  # polygon outlines only
                dLayerDefinition = UniqueValuesJSON(valList, True, labelField, True) # polygon fill, semi-transparent
                #PrintMsg(" \nFirst SoilMap dLayerDefinition used by UpdateLayerFromJSON: \n" + str(dLayerDefinition), 0)
                
                #newLayer.updateLayerFromJSON(dLayerDefinition)
                newLayer.symbology.valueField
                newLayer.symbology.updateValues()
        ...
        if arcpy.Exists("MapunitComponents"):
            # Create relate to outputFC
            #PrintMsg("\tCreating relationshipclass", 1)
            relclass = os.path.join(os.path.dirname(outputFC), "z" + os.path.basename(outputFC) + "_" + "MapunitComponents")
            arcpy.CreateRelationshipClass_management("MapunitComponents", outputFC, relclass, "SIMPLE", "", "", "NONE", "ONE_TO_MANY", "NONE", "mukey", "mukey")
...
## ===================================================================================
def AddHydricSoilMap(hydricLayerName, layerFile, labelField):
    # Create the top layer which will be simple black outline, no fill with MUSYM labels, visible
    # Run SDA query to add NATMUSYM and MAPUNIT NAME to featureclass
    try:
        ...
        newLayer = arcpy.mapping.Layer(layerFile)  #fix this
        newLayer.name = hydricLayerName  # from global variable
        ...
        fieldInfo = dict()
        soilFields = [fld.name.lower() for fld in arcpy.Describe(newLayer).fields]
        ...
        for fld in hydricFields:
            if not fld in soilFields:
                ...
                arcpy.AddField_management(newLayer, fld, fldType, "", "", fldLength)
        ...
        with arcpy.da.UpdateCursor(newLayer, hydricFields) as cur:
            for rec in cur:
                mukey = rec[0]
                ...
                if mukey in dMapunitHydric:
                    ...
                    PrintMsg("\tMissing hydric mukey " + str(mukey) +" from " + newLayer.name, 1)
        
        # AddField_management (in_table, field_name, field_type, {field_precision}, {field_scale}, {field_length}, {field_alias}, {field_is_nullable}, {field_is_required}, {field_domain})
        
        # Compare fields in newLayer vs ratingTable
        #arcpy.AddJoin_management(newLayer, "mukey", ratingTable, "mukey")
        ...
        if version[0:4] in ["10.3", "10.4", "10.5", "10.6", "10.7"]:
            ...
            newLayer.updateLayerFromJSON(dLayerDefinition)
            #newLayer.symbology.valueField = "hydric_rating"
            #PrintMsg(" \n" + str(dLayerDefinition), 1)
            #arcpy.RefreshTOC()
            #arcpy.RefreshActiveView()

        #arcpy.mapping.AddLayer(df, newLayer, "TOP")  # add soil map layer to group layer
        arcpy.mapping.AddLayerToGroup(df, grpLayer, newLayer, "BOTTOM")
...
## ===================================================================================
def CreateMapLayer(soilsFC, outputTbl, outputLayerName, outputLayerFile, outputValues, parameterString, creditsString, dLayerDefinition, bFuzzy):
    # Stolen from CreateSoilMap 2019-04-29
    #
    # Get data from table view and transfer to soils layer
    # Then create map legend and create layer and save it to a layer file

    #
    #
    try:
        # bVerbose = True
        msg = "Preparing soil map '" + outputLayerName + "'..."
        ...
        PrintMsg(str(e), 2)
        try:
            if arcpy.Exists(soilsLayer) and hasJoin:
            #    PrintMsg("\tRemoving join", 1)
                arcpy.RemoveJoin_management(soilsLayer, os.path.basename(outputTbl))
...
def AddLabels(mapLayer, labelField, bLabeled, fontSize):
    ...
    try:
        ...
        if not labelField.lower() in fieldNames:
            PrintMsg("\t" + labelField + " not found in " + desc.name + "  " + ", ".join(fieldNames) + ")", 1)
            return False
        
        #PrintMsg("\tLabel field '" + labelField + "' found in " + desc.name, 1)
        ...
        if mapLayer.supports("LABELCLASSES"):
            ...
            PrintMsg(" \n\tLayer " + mapLayer.name + " does not support labelclasses", 1)
...
## ===================================================================================
def AddFieldMap(aoiPolys, aoiField, mxd, df):
    # Add AOI layer with PartName labels to ArcMap display
    #
    try:
        PrintMsg("\nAdding Land unit map...", 0)
        boundaryLayerName = "LandUnit_Boundaries"
        ...
        #arcpy.mapping.AddLayerToGroup(df, grpLayer, aoiLayer, "TOP")
        bLabeled = AddLabels(aoiLayer, aoiField, True, 20)
...
import sys, string, os, arcpy, locale, traceback, urllib2, httplib, socket, json
...
try:
    ...
    #QueryFile = arcpy.GetParameterAsText(2)                # Query file from tool folder
    gdb = arcpy.GetParameterAsText(2)                       # Geodatabase where output tables and featureclasses may be stored
    ...
    #bPropertyQueries = arcpy.GetParameter(5)                # Run Property Query section and add to request
    #bDiagnosticQuery = arcpy.GetParameter(5)                # Add Diagnostic Query section to request. These could be soil polygon geometry, temp tables, etc.
    bSpatial = arcpy.GetParameter(5)                        # Return soil polygon geometry
    bSaveQuery = arcpy.GetParameter(6)                        # Display system messages
    #outputLayer = arcpy.GetParameter(9)                     # Place holder for interp maps
    ...
    # and then if landunit exists, the aoiShp will be unioned with the outputSoils layer.
    #
    bCleaned = CreateAOILayer(inputAOI, bDissolve, bClean, aoiShp)
    ...
    if bCleaned:
        ...
        if not bDissolve and "LANDUNIT" in fieldNames:
            bParts = True
            ...
            #PrintMsg(" \nLandunits: " + ", ".join(landunits), 1)
                    
                                        
        else:
            # Do not add landunits layer to map
            bParts = False
            
        # For each AOI polygon, send a geometry request to SDA, return attribute data only
        #
        ...
        if sQuery != "":
            ...
            grpLayerName = "SDA Thematic Soil Maps"

            # Check for existing group layer and remove if found
            layerList = arcpy.mapping.ListLayers(mxd, grpLayerName, df)
            
            if len(layerList) > 0:
                #PrintMsg(" \nRemoving group layer " + grpLayerName, 1)
            ...
            if bSpatial:
                ...
                grpLayer.visible = True
                newLayerName = "SoilMap_by_Landunit"
                layerFile = os.path.join(env.scratchFolder, "soilmaplayer.lyr")

                # Add soil mapunit layer (with relate to component information)
                newFC = os.path.join(gdb, newLayerName)
                arcpy.MakeFeatureLayer_management(newFC, newLayerName)
                arcpy.SaveToLayerFile_management(newLayerName, layerFile, "ABSOLUTE")
                time.sleep(1)
                arcpy.Delete_management(newLayerName, "FeatureLayer")

                if arcpy.Exists(newFC) and arcpy.Exists(layerFile):
                    bMapped = AddFirstSoilMap(newFC, layerFile, "musym")

                #if len(tableList) == 0:
                #    PrintMsg(" \nQuery failed to return data", 1)
                #    raise MyError, ""

            if bSpatial and "SoilMap_by_Landunit" in tableList:  # NOTE! Turning off diagnostic tables is turning off InterpMaps and HydricSoilMaps also.
                
                # Add new soil map layers generated from Soil Data Access
                #PrintMsg(" \n1. Adding new map layers or tables: "  + ", ".join(tableList), 1)
                

                # Try getting soilsLayer here  "SoilMap_by_Landunit"
                soilsLayerList = arcpy.mapping.ListLayers(mxd, newLayerName, df)

                if len(soilsLayerList) > 0:
                    soilsLayer = soilsLayerList[0]

                else:
                    raise MyError, "Could not get soilsLayer object"

                #PrintMsg(" \nCalling CreateInterpMaps...", 1)     
                bInterps = CreateInterpMaps(gdb, soilsLayer, tableList)

                # First create aoi boundary map layer using the input aoi polygon featureclass
                if bParts:
                    if arcpy.Exists(aoiShp):
                        # new AOI layer
                        #PrintMsg("\tAdding AOI boundary layer...", 0)
                        bFields = AddFieldMap(aoiShp, "landunit", mxd, df)

                # Add soil map layer(s) after AOI layer

                if "MapunitHydricInterp" in tableList:
                    PrintMsg("\tAdding hydric soil map...", 0)
                    # Create a soil map for Jason's hydric interpretation
                    hydricLayerName = "HydricSoilMap"
                    bMapped = AddHydricSoilMap(hydricLayerName, layerFile, "hydric_rating")

            else:
                #PrintMsg(" \n2. 'SoilMap_by_Landunit' not found in tableList: "  + ", ".join(tableList), 1)
                pass
