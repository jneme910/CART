/** SDA Query application='CART' rule='Generalized Resource Assessment - June 2025' version='FY25' **/
-- 2021-09-27 T15:35:41 Edited: 06/27/2025
-- Added Order 5 Soils Restriction; Jason Nemecek ; June 27, 2025
-- Fixed insert into #AoiSoils2 - added MakeValid ;  Susan McGlasson ;  April 30, 2024
-- Fixed 'Farm Land Class' query - return 0 instead on 'not rated' ; Susan McGlasson ; 05/31/2023
-- Updated Soil Organic Carbon Stock Domain and added Hydric Easements to Air Quality 09/27/2021
-- Includes Air Quality 09/23/2021
-- Limitations for Aerobic Soil Organisms 8/2021




/*
MultiPolygon (((-150.36696675326803074 60.57729328035275529, -150.36696675326803074 60.85319098345309641, -145.81465465211246624 60.85319098345309641, -145.81465465211246624 60.57729328035275529, -150.36696675326803074 60.57729328035275529)))

MultiPolygon (((-151.55014344156364814 61.8825016450197225, -151.55014344156364814 62.16901079823930587, -149.86292287260391731 62.16901079823930587, -149.86292287260391731 61.8825016450197225, -151.55014344156364814 61.8825016450197225)))

MultiPolygon (((-151.5819777919213891 61.68088409275409845, -151.5819777919213891 61.78699859394653515, -145.98974357907999888 61.78699859394653515, -145.98974357907999888 61.68088409275409845, -151.5819777919213891 61.68088409275409845)))

MultiPolygon (((-151.48746956429695842 64.63749938222836988, -151.48746956429695842 64.65540620430461161, -151.38533435689922158 64.65540620430461161, -151.38533435689922158 64.63749938222836988, -151.48746956429695842 64.63749938222836988)))
...
DROP TABLE IF EXISTS #AllOrder6
...
SELECT @aoiGeom = GEOMETRY::STGeomFromText('MULTIPOLYGON (((-150.36696675326803074 60.57729328035275529, -150.36696675326803074 60.85319098345309641, -145.81465465211246624 60.85319098345309641, -145.81465465211246624 60.57729328035275529, -150.36696675326803074 60.57729328035275529)))', 4326); 
SELECT @aoiGeomFixed = @aoiGeom.MakeValid().STUnion(@aoiGeom.STStartPoint()); 
INSERT INTO #AoiTable ( landunit, aoigeom )  
VALUES ('T9981 Fld3', @aoiGeomFixed); 
SELECT @aoiGeom = GEOMETRY::STGeomFromText('MULTIPOLYGON (((-151.48746956429695842 64.63749938222836988, -151.48746956429695842 64.65540620430461161, -151.38533435689922158 64.65540620430461161, -151.38533435689922158 64.63749938222836988, -151.48746956429695842 64.63749938222836988)))', 4326); 
SELECT @aoiGeomFixed = @aoiGeom.MakeValid().STUnion(@aoiGeom.STStartPoint()); 
INSERT INTO #AoiTable ( landunit, aoigeom )  
...
INSERT INTO #AoiSoils2
    SELECT aoiid, polyid, landunit, mukey, 
...
INSERT INTO #M2
    ...
    ROUND (SUM (M1.poly_acres) OVER(PARTITION BY M1.landunit, M1.mukey), 3) AS mapunit_acres, m.invesintens
    FROM #AoiSoils2 AS M1
	INNER JOIN mapunit AS m ON m.mukey = M1.mukey
    GROUP BY M1.aoiid, M1.landunit, M1.mukey, M1.poly_acres, m.invesintens
;

---############################################################
----Universal "Order 5 Not Rated" Logic for Map Units

--- Universal "Order 5 Not Rated" Logic for Map Units
/*This SQL script implements a comprehensive workflow for soil data analysis and reporting, including the creation of temporary tables, spatial AOI processing, soil property calculations, and soil interpretation ratings. It incorporates a universal rule to mark any map unit with invesintens = 'Order 5' as "Not rated" across all relevant queries and outputs. .*/
---############################################################
IF OBJECT_ID('tempdb..#AllOrder6') IS NOT NULL DROP TABLE #AllOrder6;
SELECT landunit, mukey, 
       CASE WHEN COUNT(*) = SUM(CASE WHEN invesintens = 'Order 5' THEN 1 ELSE 0 END)
            THEN 1 ELSE 0 END AS is_order5
INTO #AllOrder6
FROM #M2
GROUP BY landunit, mukey;



-- Populate #FC table with Farm Land Class
-- Fixed 'Farm Land Class' query - return 0 instead on 'not rated' 
INSERT INTO #FC
    SELECT fcc.aoiid, fcc.landunit, mu.mukey, mapunit_acres,
    CASE WHEN mu.invesintens = 'Order 5' THEN 'Not rated' --06/27/2025
		WHEN farmlndcl IS NULL  THEN 'Not rated'
        ...
        ELSE 'Not Prime'
    END AS farmlndclass,
	    CASE  WHEN mu.invesintens = 'Order 5' THEN 0 --06/27/2025
		WHEN farmlndcl IS NULL  THEN 0   -------<<<<<<<<<<<<< sdm
...
SELECT DISTINCT #AoiAcres.landunit, 'Farm Class' AS attributename, 
CASE WHEN farm_ac_sum IS NULL THEN 2
WHEN farm_ac_sum > 0 THEN 1 ELSE 0 END AS rating_value, CASE 
WHEN farm_ac_sum IS NULL THEN 'Not Rated'
WHEN farm_ac_sum > 0 THEN 'Yes' ELSE 'No' END AS rating_class 
FROM #AoiAcres  
...
-- Easements
INSERT INTO #LandunitRatingsCART2 (landunit, attributename, rating_value, rating_class)
SELECT DISTINCT #AoiAcres.landunit, 'Easements Farm Class' AS attributename, 
CASE WHEN farm_ac_sum IS NULL THEN 2
WHEN farmac_sum_pct >= 50 THEN 1 ELSE 0 END AS rating_value, CASE 
WHEN farm_ac_sum IS NULL THEN 'Not Rated'
WHEN farmac_sum_pct>= 50 THEN 'Yes' ELSE 'No' END AS rating_class 
FROM #AoiAcres  
LEFT OUTER JOIN #FC2 ON #AoiAcres.aoiid=#FC2.aoiid
;
...
major_mu_pct_sum,
mu_pct_sum,
CASE WHEN is_order5 = 1 THEN NULL ELSE SUM (adj_comp_pct * comp_weighted_average_l) OVER(PARTITION BY ag6.mukey, aoiid ) END AS MU_SUM_AGG_L,
CASE WHEN is_order5 = 1 THEN NULL ELSE SUM (adj_comp_pct * comp_weighted_average_r) OVER(PARTITION BY ag6.mukey, aoiid ) END AS MU_SUM_AGG_R,
CASE WHEN is_order5 = 1 THEN NULL ELSE SUM (adj_comp_pct * comp_weighted_average_h) over(PARTITION BY ag6.mukey, aoiid ) END AS MU_SUM_AGG_H
--(mapunit_acres/landunit_acres)*MU_SUM_AGG_R AS MU_Weighted_Average_R
FROM #agg6 AS ag6
INNER JOIN #AllOrder6 AS a ON a.mukey=ag6.mukey
GROUP BY aoiid ,
ag6.landunit,
landunit_acres,
ag6.mukey,
...
TRUNCATE TABLE #M6
INSERT INTO #M6
SELECT aoiid, #M5.landunit,#M5.mukey, mapunit_acres, cokey, compname, comppct_r, 
CASE WHEN a.is_order5= 1 THEN 'Not Rated' ELSE  rating_class END AS rating_class  , mu_pct_sum, (1.0 * comppct_r / NULLIF(mu_pct_sum, 0)) AS adj_comp_pct
FROM #M5
INNER JOIN #AllOrder6 AS a ON a.mukey=#M5.mukey
;
...
-- Begin query for soil interpretation:  Limitations for Aerobic Soil Organisms
-- ************************************************************************************************

SELECT @attributeName = 'Limitations for Aerobic Soil Organisms';
SELECT @minPct = 10;
SELECT @minAcres = 10;
...
-- End of:  Limitations for Aerobic Soil Organisms
-- ************************************************************************************************
...
IF OBJECT_ID('tempdb..#AllOrder5') IS NOT NULL DROP TABLE #AllOrder5;
SELECT landunit, mukey, 
       CASE WHEN COUNT(*) = SUM(CASE WHEN invesintens = 'Order 5' THEN 1 ELSE 0 END)
            THEN 1 ELSE 0 END AS is_order5
INTO #AllOrder5
...
CREATE TABLE #LandunitRatingsAirQualityData (
    landunit CHAR(20),
    rating_name_CS CHAR(60),
	rating_value_CS INT,
    rating_class_CS CHAR(60),
	rating_name_OMD CHAR(60),
	rating_value_OMD INT,
    rating_class_OMD CHAR(60),
	    rating_name_HYD CHAR(60),
	rating_value_HYD INT,
    rating_class_HYD CHAR(60),
	)


INSERT INTO #LandunitRatingsAirQualityData
(
    landunit,
    rating_name_CS,
    rating_value_CS,
...
SELECT LRC.landunit, LRC.attributename AS rating_name, LRC.rating_value, LRC.rating_class,
 LRC2.attributename AS rating_name2, LRC2.rating_value AS rating_value2, LRC2.rating_class AS rating_class2,
  LRC3.attributename AS rating_name3, LRC3.rating_value AS rating_value3, LRC3.rating_class AS rating_class3
...
--------------------------------------------------------- begin long sql
INSERT INTO  #LandunitRatingsCART2 (landunit, attributename, rating_value)
SELECT landunit, 'Air Quality' AS rating_name, 
-----------------
----------------- class 1 
CASE WHEN (rating_value_CS = 2 AND rating_value_OMD = 5  AND rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 2 AND rating_value_OMD = 4  and rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 2 AND rating_value_OMD = 3  and rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 2 AND rating_value_OMD = 2  and rating_value_HYD in ( 2,0))
THEN 1 
-----------------
WHEN (rating_value_CS = 3 AND rating_value_OMD = 5  and rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 3 AND rating_value_OMD = 4  and rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 3 AND rating_value_OMD = 3  and rating_value_HYD in ( 2,0))
THEN 1 
-----------------
WHEN (rating_value_CS = 4 AND rating_value_OMD = 5  and rating_value_HYD in ( 2,0))
THEN 1 
WHEN (rating_value_CS = 4 AND rating_value_OMD = 4  and rating_value_HYD in ( 2,0))
THEN 1 
-----------------
WHEN (rating_value_CS = 5 AND rating_value_OMD = 5  and rating_value_HYD in ( 2,0))
THEN 1 
-----------------
----------------- class 2
WHEN (rating_value_CS = 2 AND rating_value_OMD = 1  and rating_value_HYD in ( 2,0))
THEN 2
----------------
WHEN (rating_value_CS = 3 AND rating_value_OMD = 2  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 3 AND rating_value_OMD = 1  and rating_value_HYD in ( 2,0))
THEN 2
----------------
WHEN (rating_value_CS = 4 AND rating_value_OMD = 3  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 4 AND rating_value_OMD = 2  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 4 AND rating_value_OMD = 1  and rating_value_HYD in ( 2,0))
THEN 2
----------------
WHEN (rating_value_CS = 5 AND rating_value_OMD = 4  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 5 AND rating_value_OMD = 3  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 5 AND rating_value_OMD = 2  and rating_value_HYD in ( 2,0))
THEN 2
----------------
WHEN (rating_value_CS = 6 AND rating_value_OMD = 5  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 6 AND rating_value_OMD = 4  and rating_value_HYD in ( 2,0))
THEN 2
WHEN (rating_value_CS = 6 AND rating_value_OMD = 3  and rating_value_HYD in ( 2,0))
THEN 2
----------------- class 3
WHEN (rating_value_CS = 5 AND rating_value_OMD = 1  and rating_value_HYD in ( 2,0))
THEN 3
WHEN (rating_value_CS = 6 AND rating_value_OMD = 2  and rating_value_HYD in ( 2,0))
THEN 3
WHEN (rating_value_CS = 6 AND rating_value_OMD = 1  and rating_value_HYD in ( 2,0))
THEN 3
--------------
-------------- make class = 3 when hydric
WHEN (rating_value_HYD = 1)
THEN 3
--------------
-------------- else make nNOT rated
ELSE  4
END AS rating_value
