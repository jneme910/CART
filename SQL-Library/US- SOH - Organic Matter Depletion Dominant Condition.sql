 ...
 INNER JOIN cointerp ON component.cokey = cointerp.cokey AND mapunit.mukey = mu.mukey AND ruledepth = 0 AND mrulename LIKE 'SOH - Organic Matter Depletion' GROUP BY interphrc, interphr
 ORDER BY SUM (comppct_r) DESC)as rating,
 (SELECT TOP 1 interphrc
 FROM mapunit
 INNER JOIN component ON component.mukey=mapunit.mukey
 INNER JOIN cointerp ON component.cokey = cointerp.cokey AND mapunit.mukey = mu.mukey AND ruledepth = 0 AND mrulename LIKE 'SOH - Organic Matter Depletion'
...
AND ruledepth != 0 AND interphrc NOT LIKE 'Not%' AND mrulename LIKE 'SOH - Organic Matter Depletion' GROUP BY interphrc, interphr
