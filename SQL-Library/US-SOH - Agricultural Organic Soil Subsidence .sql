 ...
 INNER JOIN cointerp ON component.cokey = cointerp.cokey AND mapunit.mukey = mu.mukey AND ruledepth = 0 AND mrulename LIKE 'SOH - Agricultural Organic Soil Subsidence' GROUP BY interphrc, interphr
 ORDER BY SUM (comppct_r) DESC)as rating,
 (SELECT TOP 1 interphrc
 FROM mapunit
 INNER JOIN component ON component.mukey=mapunit.mukey
 INNER JOIN cointerp ON component.cokey = cointerp.cokey AND mapunit.mukey = mu.mukey AND ruledepth = 0 AND mrulename LIKE 'SOH - Agricultural Organic Soil Subsidence'
 GROUP BY interphrc, comppct_r ORDER BY SUM(comppct_r) over(partition by interphrc) DESC) as class,
 
 LEFT ( (SELECT DISTINCT SUBSTRING(  (  SELECT ( '; ' + interphrc)
FROM mapunit
...
AND ruledepth != 0 AND interphrc NOT LIKE 'Not%' AND mrulename LIKE 'SOH - Agricultural Organic Soil Subsidence' GROUP BY interphrc, interphr
ORDER BY interphr DESC, interphrc
FOR XML PATH('') ), 3, 1000) ), 200) as reason
